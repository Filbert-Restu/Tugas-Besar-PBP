...
if __name__ == '__main__':
    ...
    if args.resume:
        ...
        logging.info("Init from base net %s", args.base_net)
        try:
            net.init_from_base_net(args.base_net)
        except RuntimeError:
            logging.info("Base net load failed, mencoba ambil backbone saja...")
            state_dict = torch.load(args.base_net, map_location="cpu")
            base_dict = {k.replace("base_net.", "").replace("features.", ""): v
                        for k, v in state_dict.items() if k.startswith("base_net.") or k.startswith("features.")}
            net.base_net.load_state_dict(base_dict, strict=False)
